...
Proportion_removed$Habitat_type <- factor(Proportion_removed$Habitat_type, levels=c("Barnacle", "Bare_rock"))
...
updated_bare_vs_barn_removal <- ggplot(aes(x = Habitat_type, y = bare_vs_barn_proportion_removed, fill = Habitat_type, colour = Habitat_type, alpha = 0.3), data = Proportion_removed) +
  #geom_point(aes(shape = Habitat_type), size = 3) +
  scale_shape_manual(values=c(18,17))+
  guides(
    colour = guide_legend(override.aes = list(shape = c(18,17))),
    shape = "none"
  ) +
  geom_boxplot() + 
  scale_fill_manual(values = c("grey75", "darkorange2"))+
  geom_jitter()+
  scale_colour_manual(values =c("grey75", "darkorange2"))+
  theme_326 + # add the pre-defined theme
  labs(y = "Proportion Littorina Removed") + # labels
  theme(legend.position = "none")
updated_bare_vs_barn_removal

p1 <- updated_bare_vs_barn_removal+ scale_x_discrete(labels=c("Barnacle" = "Barnacle", "Bare_rock" = "Bare rock")) + theme(axis.title.x = element_blank())
p1

#Plot keeping colour scheme of other figures

Proportion_removed_invson_ttest$Habitat_type <- factor(Proportion_removed_invson_ttest$Habitat_type, levels=c("On", "In"))

in_vs_on_removal <- ggplot(aes(x = Habitat_type, y = in_vs_on_proportion_removed, fill = Habitat_type, colour = Habitat_type, alpha = 0.3), data = Proportion_removed_invson_ttest) +
  geom_boxplot() + 
  scale_fill_manual(values = c("steelblue4", "darkolivegreen4"))+
  geom_jitter()+
  scale_colour_manual(values = c("steelblue4", "darkolivegreen4"))+
  ylim(min=0, max=1)+
  theme_326 + # add the pre-defined theme
  labs(y = "Proportion Littorina Removed") + # labels
  theme(legend.position = "none")
...
#ggsave("Figures/UniqueColours.png", device = "png", height = 9, width = 12, dpi = 400)
```
